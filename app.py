        ...
        try:
            ...
            for detection in detections:
                ...
                try:
                    ...
                    # Rotate the flooring image before applying
                    if categoryname == "Flooring":
                        angle = 90  # Rotation angle in degrees (change as needed)
                        center = (new_image.shape[1] // 2, new_image.shape[0] // 2)
                        matrix = cv2.getRotationMatrix2D(center, angle, 1.0)
                        rotated_image = cv2.warpAffine(new_image, matrix, (new_image.shape[1], new_image.shape[0]))

                        # Optionally, crop the rotated image to a bounding box or resize as necessary
                        x, y, w, h = cv2.boundingRect(m.astype(np.uint8))
                        resized_rotated_image = cv2.resize(rotated_image, (w, h))
                        img[y:y+h, x:x+w] = np.where(m[y:y+h, x:x+w, np.newaxis], resized_rotated_image, img[y:y+h, x:x+w])

                    else:
                        # For other categories, apply image as is
                        x, y, w, h = cv2.boundingRect(m.astype(np.uint8))
